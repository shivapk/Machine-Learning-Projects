            ...
            for r in range(1,5):
                ...
                for g in range(1,5):
                    ...
                    if acc>max:
                        ...
                        best_r=r
                        best_g=g
    print ('Best C is %s , Best d is %s, Best r is %s, Best g is %s, Cross Validation Accuracy is %f'%(best_c,best_d,best_r,best_g,max))
    return [best_c,best_d,best_r,best_s]
...
start=time.time()    
b_c,b_d,b_r,b_g=cross_val(y_train,x_train)
end=time.time()
print ('Average Training time = %f seconds'%((end-start)/400))

polynomial_svm(y_test,x_test,b_c,b_d,b_r,b_g)
